...
import (
	...
	"github.com/hgsgtk/ngrok/internal/msg"

	"github.com/hgsgtk/ngrok/internal/util"

	"github.com/hgsgtk/ngrok/internal/proto"
...
func newClientModel(config *Configuration, ctl mvc.Controller) *ClientModel {
	...
	m := &ClientModel{
		Logger: ngrok.NewPrefixLogger("client"),
...
func (c ClientModel) GetClientVersion() string       { return ngrok.MajorMinor() }
...
func (c *ClientModel) PlayRequest(tunnel mvc.Tunnel, payload []byte) {
	var localConn ngrok.Conn
	localConn, err := ngrok.Dial(tunnel.LocalAddr, "prv", nil)
	if err != nil {
...
func (c *ClientModel) control() {
	defer func() {
		if r := recover(); r != nil {
			ngrok.Error("control recovering from failure %v", r)
		}
	}()
	...
	if c.proxyUrl == "" {
		...
		ctlConn, err = ngrok.Dial(c.serverAddr, "ctl", c.tlsConfig)
	} else {
		ctlConn, err = ngrok.DialHttpProxy(c.proxyUrl, c.serverAddr, "ctl", c.tlsConfig)
	}
	if err != nil {
		panic(err)
	}
...
func (c *ClientModel) proxy() {
	...
	if err != nil {
		ngrok.Error("Failed to establish proxy connection: %v", err)
		return
	}
...
%s`, len(badGatewayBody), badGatewayBody)))
	...
	m.connTimer.Time(func() {
		...
		bytesIn, bytesOut := ngrok.Join(localConn, remoteConn)
...
func (c *ClientModel) heartbeat(lastPongAddr *int64, conn ngrok.Conn) {
