...
func readMsgShared(c ngrok.Conn) (buffer []byte, err error) {
...
func ReadMsg(c ngrok.Conn) (msg Message, err error) {
...
func ReadMsgInto(c ngrok.Conn, msg Message) (err error) {
	buffer, err := readMsgShared(c)
	if err != nil {
		return
	}
...
func WriteMsg(c ngrok.Conn, msg interface{}) (err error) {
	buffer, err := Pack(msg)
	if err != nil {
		return
	}
