...
func startHttpListener(addr string, tlsCfg *tls.Config) (listener *ngrok.Listener) {
	// bind/listen for incoming connections
	var err error
	if listener, err = ngrok.Listen(addr, "pub", tlsCfg); err != nil {
		panic(err)
	}
	...
	ngrok.Info("Listening for public %s connections on %v", proto, listener.Addr.String())
	go func() {
...
func httpHandler(c ngrok.Conn, proto string) {
	defer c.Close()
	defer func() {
