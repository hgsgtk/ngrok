...
type Metrics interface {
	ngrok.Logger
	OpenConnection(*Tunnel, ngrok.Conn)
	CloseConnection(*Tunnel, ngrok.Conn, time.Time, int64, int64)
	OpenTunnel(*Tunnel)
	CloseTunnel(*Tunnel)
}
...
func NewLocalMetrics(reportInterval time.Duration) *LocalMetrics {
	metrics := LocalMetrics{
		Logger:         ngrok.NewPrefixLogger("metrics"),
...
func (m *LocalMetrics) OpenConnection(t *Tunnel, c ngrok.Conn) {
	m.connMeter.Mark(1)
}

func (m *LocalMetrics) CloseConnection(t *Tunnel, c ngrok.Conn, start time.Time, bytesIn, bytesOut int64) {
	m.bytesInCount.Inc(bytesIn)
	m.bytesOutCount.Inc(bytesOut)
}

func (m *LocalMetrics) Report() {
	ngrok.Info("Reporting every %d seconds", int(m.reportInterval.Seconds()))
	...
	for {
		...
		if err != nil {
			ngrok.Error("Failed to serialize metrics: %v", err)
			continue
		}

		ngrok.Info("Reporting: %s", buffer)
	}
}
...
func NewKeenIoMetrics(batchInterval time.Duration) *KeenIoMetrics {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				k.Logger.Error("KeenIoMetrics failed: %v", r)
			}
		}()
		...
		for {
			...
			case <-batchTimer:
				...
				if err != nil {
					k.Logger.Error("Failed to serialize metrics payload: %v, %v", batch, err)
				} else {
					for key, val := range batch {
						ngrok.Debug("Reporting %d metrics for %s", len(val), key)
					}
...
func (k *KeenIoMetrics) AuthedRequest(method, path string, body *bytes.Reader) (resp *http.Response, err error) {
	...
	if err != nil {
		ngrok.Error("Failed to send metric event to keen.io %v", err)
	} else {
		ngrok.Info("keen.io processed request in %f sec", time.Since(requestStartAt).Seconds())
		defer resp.Body.Close()
		if resp.StatusCode != 200 {
			bytes, _ := ioutil.ReadAll(resp.Body)
			ngrok.Error("Got %v response from keen.io: %s", resp.StatusCode, bytes)
		}
	}
...
func (k *KeenIoMetrics) OpenConnection(t *Tunnel, c ngrok.Conn) {
}

func (k *KeenIoMetrics) CloseConnection(t *Tunnel, c ngrok.Conn, start time.Time, in, out int64) {
	event := struct {
