...
func NewControl(ctlConn ngrok.Conn, authMsg *msg.Auth) {
	var err error
	...
	c := &Control{
		...
		proxies:         make(chan ngrok.Conn, 10),
		lastPing:        time.Now(),
	...
	if authMsg.Version != ngrok.Proto {
		failAuth(fmt.Errorf("Incompatible versions. Server %s, client %s. Download a new version at http://ngrok.com", ngrok.MajorMinor(), authMsg.Version))
		return
	}
...
func (c *Control) RegisterProxy(conn ngrok.Conn) {
	conn.AddLogPrefix(c.id)

	conn.SetDeadline(time.Now().Add(proxyStaleDuration))
	select {
	case c.proxies <- conn:
		ngrok.Info("Registered")
	default:
		ngrok.Info("Proxies buffer is full, discarding.")
		conn.Close()
	}
}
...
func (c *Control) GetProxy() (proxyConn ngrok.Conn, err error) {
	var ok bool
