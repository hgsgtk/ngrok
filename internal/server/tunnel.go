...
func (t *Tunnel) listenTcp(listener *net.TCPListener) {
	for {
		defer func() {
			if r := recover(); r != nil {
				ngrok.Warn("listenTcp failed with error %v", r)
			}
		}()
...
func (t *Tunnel) HandlePublicConnection(publicConn ngrok.Conn) {
	...
	bytesIn, bytesOut := ngrok.Join(publicConn, proxyConn)
